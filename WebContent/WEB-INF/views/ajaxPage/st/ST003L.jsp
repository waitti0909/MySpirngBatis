...
// 				}
				...
				$('#btn_st_assign').click(function() {
					...
					if (id!="" && id!=null) {
						...
						assignPage(row.work_ID ,row.order_ID, "edit",row.rep_DEPT,row.deptName);		
					}else {
			        	alert("先選取一列資料列");
			        }		 		
				});
				...
				$("#grid").jqGrid({
					datatype: "local",
					pager: '#pager',
					url : CONTEXT_URL+"/st/st003/search",
				   	colNames:['作業編號','工務類別','派工目的','基站編號','基站名稱','工單號碼','接工單位','負責人','狀態','rep_DEPT','pick_USER','end_DESC','odr_STATUS'],
				   	colModel:[				   
						{name:'work_ID',index:'work_ID',width:90, sortable: false},
						{name:'workTypeName',index:'workTypeName',width:100, sortable: false},
			...
			//派工
			function assignPage (workId ,orderId, status ,rep_DEPT ,deptName) {
				$.fancybox.open({
					href : CONTEXT_URL+"/st/st003/assignPage",
		 			type : 'ajax',
		 			width : $(window).width(),
		 			height : $(window).width(),
		 			autoSize : false,
			...
			function updateEide(workId, orderId, pickUser, endDesc, status,
					orderTypeName, orderStatus){
				$.fancybox.open({
		 			href : CONTEXT_URL+"/st/st003/updatePage",
		 			type : 'ajax',
		 			width : $(window).width(),
		 			height : $(window).width(),
		 			autoSize : false,
			...
			<div class="col-xs-12 col-sm-12">
				<div class="box ui-draggable ui-droppable">
					<div class="box-header">
						<div class="box-name">
							<i class="fa fa-search"></i> <span>尋點作業處理 工單查詢</span>
						</div>
						<div class="box-icons">
