		...
		<script type="text/javascript">
			...
			function mountGrid(){
				...
				$(window).resize(function() {
					...
					$("#inv008Grid,.grid").setGridWidth($("#inv008JQgrid").width() - 10);
			...
			function mountButEvent(){
				...
				$('#btn_search').click(function(){
					...
					if(checkDateStrRange($("#strDate").val(), $("#endDate").val())) {
						...
						$("#inv008Grid").setGridParam({datatype : "json",postData : data,page:1}).trigger("reloadGrid");
					}else{
				...
				//調出按鈕
				$('#btn_tran_out').click(function(){
					
					var id = $('#inv008Grid').jqGrid('getGridParam', 'selrow');
					...
					//調撥進度為(3.核可，4:部份調出)的資料才可進入
					if(invTrData.statusValue == '核可' || invTrData.statusValue == '部份調出'){
 						exportDataEvent(invTrData);
					}else{
						alert("調撥進度為 [ "+invTrData.statusValue+" ] 無法調出。");
					}
				});
			}
			...
			function exportDataEvent(invTrData){
				$.fancybox.open({
					href : CONTEXT_URL + "/inv/inv008/exportDataDetailByDto",
					type : 'ajax',
			        width : $(window).width(),
			...
			function doubleClickEvent(invTrData){
				$.fancybox.open({
					href : CONTEXT_URL + "/inv/inv008/exportDataDetailByView",
					type : 'ajax',
			        width : $(window).width(),
			        height : $(window).height(),
					autoSize : false,
	...
	<body>
		...
		<div class="row">
			...
			<div id="ajaxSearchResult" class="col-xs-12">
				<div id="inv008JQgrid">
					<table id="inv008Grid"></table>
					<div id="inv008Pager"></div>
				</div>
			</div>
