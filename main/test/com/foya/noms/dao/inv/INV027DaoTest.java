...
public class INV027DaoTest extends GenericTest {
	...
	public void testSelectEqpModelByExample() {
		List<TbComEqpModel> list = inv027Dao.selectEqpModelByExample(new TbComEqpModelExample());
	...
	public void testSelectEqpTypeByExample() {
		...
		List<TbComEqpType> list = inv027Dao.selectEqpTypeByExample(example2);
		Assert.assertNotNull(list);
		Assert.assertTrue(list.size() > 0);
	}
	...
	public void testSelectDistinctInvBomByExample() {
		...
		List<TbInvBomDTO> list = inv027Dao.selectDistinctInvBomByExample(example);
		Assert.assertNotNull(list);
		Assert.assertTrue(list.size() > 0);
	}
	...
	public void testSelectInvBomByExample() {
		...
		cr.andEqp_model_idEqualTo("1A");
		cr.andEqp_type_idEqualTo("1");
		List<TbInvBomDTO> list = inv027Dao.selectInvBomByExample(example);
		Assert.assertNotNull(list);
		Assert.assertTrue(list.size() > 0);
	}
	...
	public void testCountInvBomByExample() {
		...
		int count = inv027Dao.countInvBomByExample(example);
	...
	public void testInsertInvBomSelective() {
		...
		int status = inv027Dao.insertInvBomSelective(tb);
	...
	public void testUpdateInvBomByPrimaryKeySelective() {
		...
		tb.setMd_user("JUnitTestCase");
		tb.setMd_time(new Date());
		int status = inv027Dao.updateInvBomByPrimaryKeySelective(tb);
		Assert.assertTrue(status == 1);
	}
	...
	public void testSelectInvMaterialByExample() {
		...
		List<TbInvMaterialDTO> list = inv027Dao.selectInvMaterialByExample(example);
		Assert.assertNotNull(list);
		Assert.assertTrue(list.size() > 0);
	}
	...
	public void testSelectInvCategoryByExample() {
		...
		cr.andCatg1_codeEqualTo("31");
		cr.andCatg2_codeEqualTo("001");
		List<TbInvCategory> list = inv027Dao.selectInvCategoryByExample(example);
		Assert.assertNotNull(list);
		Assert.assertTrue(list.size() > 0);
	}
